...
- The remaining open milestone question is no longer the docker-first verdict itself, but whether v2.4 should be archived immediately under the documented residual-debt posture or wait for the remaining Phase 23 browser UAT to be cleared.
...
- v2.4 is conditionally ready for archival: the final verdict is explicit and machine-checked, but the open Phase 23 browser-UAT debt still determines whether the team wants immediate archival or one more verification pass first.
...
- [ ] Decide whether to archive v2.4 now under the documented residual-debt posture or clear Phase 23 browser-visible validation-truth UAT first.
- [ ] Keep the archived milestone summary honest about the final `optional` verdict and its fixed-slice evidence boundary.
