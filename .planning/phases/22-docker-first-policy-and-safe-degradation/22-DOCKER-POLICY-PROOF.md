...
Phase 22 closes on `.planning/phases/22-docker-first-policy-and-safe-degradation/22-docker-policy-slice.json`, which freezes a four-case contract for the new `llm` routing policy:
...
- `contracts/docker-bypass-fallback/snippet.py` keeps requested policy `docker-first`, requires first hop `env`, and requires `docker_bypass_reason: docker unavailable` plus `docker-bypass.txt`.
...
After Plan 22-03, reviewers should require all of these conditions:
...
- Docker-bypass fallback cases record requested policy `docker-first`, keep `llm_validation_route: env-first-docker-bypass`, record `docker_bypass_reason: docker unavailable`, and leave `docker-bypass.txt`
